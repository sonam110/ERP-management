        ...
        <div class="col-sm-12">
            <div class="row">
                <div class="col-xl-3">
                    <div class="card sticky-top" style="top:30px">
                        <div class="list-group list-group-flush" id="lead-sidenav">
                            ...
                            @endif
                            @if(Auth::user()->type != 'client')
                                <a href="#followup" class="list-group-item list-group-item-action border-0">{{__('Follow Ups')}}
                                    <div class="float-end"><i class="ti ti-chevron-right"></i></div>
                                </a>
                            @endif
                            @if(Auth::user()->type != 'client')
                ...
                <div class="col-xl-9">
                    ...
                    <div id="followup" class="card">
                        <div class="card-header">
                            <div class="d-flex align-items-center justify-content-between">
                                <h5>{{__('Follow Ups')}}</h5>

                                <div class="float-end">
                                <a data-size="lg" data-url="{{ route('leads.follow_up',$lead->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Add Follow Up')}}" class="btn btn-sm btn-primary">
                                    <i class="ti ti-plus text-white"></i>
                                </a>
                            </div>
                            </div>
                                ...
                                <table class="table table-hover mb-0">
                                    ...
                                    @foreach($followups as $followup)
                                        <tr>
                                            ...
                                            <td>{{ $followup->date }}</td>
                                            <td>{{ $followup->time }}</td>
                                            <td>{{ isset($followup->pipeline) ? $followup->pipeline->name : '-' }}</td>
                                            <td>{{ isset($followup->stage) ? $followup->stage->name : '-' }}</td>
                                             <td>{{ ($followup->status=='1') ? 'Closed' : 'Opened' }}</td>
                                            <td>
                                                @can('edit lead call')
                                                    <div class="action-btn bg-info ms-2">
                                                        <a href="#" class="mx-3 btn btn-sm d-inline-flex align-items-center" data-url="{{ URL::to('leads/'.$lead->id.'/follow_up/'.$followup->id.'/edit') }}" data-ajax-popup="true" data-size="lg" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-title="{{__('Edit Follow Up')}}">
                                                            <i class="ti ti-pencil text-white"></i>
                                                        </a>
                                                    </div>
                                                @endcan
                                                @can('delete lead call')
                                                    <div class="action-btn bg-danger ms-2">
                                                        {!! Form::open(['method' => 'DELETE', 'route' => ['leads.follou_up.destroy', $lead->id,$followup->id],'id'=>'delete-form-'.$lead->id]) !!}
                                                        <a href="#" class="mx-3 btn btn-sm  align-items-center bs-pass-para" data-bs-toggle="tooltip" title="{{__('Delete')}}"><i class="ti ti-trash text-white"></i></a>
