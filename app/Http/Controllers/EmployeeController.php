...
class EmployeeController extends Controller
{
    ...
    public function create()
    {
        if(\Auth::user()->can('create employee'))
        {
            ...
            /*-----Sonam------------*/
            $roles = Role::where('created_by', '=', \Auth::user()->creatorId())->where('name', '!=', 'client')->get()->pluck('name', 'id');
...
//            $employeesId  = \Auth::user()->employeeIdFormat($employee->employee_id);
            ...
            /*-----Sonam------------*/
            $role = User::find($employee->user_id);
            $role_d = Role::where('name',$role->type)->where('created_by', \Auth::user()->creatorId())->first();
            $roles = Role::where('created_by', '=', \Auth::user()->creatorId())->where('name', '!=', 'client')->get()->pluck('name', 'id');
            return view('employee.edit', compact('employee', 'employeesId', 'branches', 'departments', 'designations', 'documents','departmentData','roles','role_d'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
